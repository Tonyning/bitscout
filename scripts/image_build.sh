...
sudo grub-mkrescue --modules="iso9660 linux ext2 part_gpt fshelp ls boot jpeg video_bochs video_cirrus" --output=./$PROJECTNAME-16.04.iso ./image -- -r -volid "${PROJECTNAME}-${GLOBAL_BUILDID}" -partition_offset 16 -J -l -joliet-long -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -no-emul-boot
