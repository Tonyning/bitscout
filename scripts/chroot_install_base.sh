...
apt-fast --yes -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" install grub-pc ntpdate && exit 0 || exit 1' || exit 1
else
  chroot_exec chroot 'export DEBIAN_FRONTEND=noninteractive
apt-get --yes install -f localepurge aria2 &&
aria2c(){ /usr/bin/aria2c --console-log-level=warn "$@";} && export -f aria2c &&
...
apt-fast --yes -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" install grub-pc qemu-kvm && exit 0 || exit 1' || exit 1
fi &&
...
aria2c(){ /usr/bin/aria2c --console-log-level=warn "$@";} && export -f aria2c &&
apt-fast --yes -o DPkg::options::="--force-confdef" -o DPkg::options::="--force-confold" upgrade' || exit 1 &&
